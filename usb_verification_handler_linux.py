...
def block_keyboard(timeout=None):
    global keyboard_blocked
    try:
        with keyboard_lock:
            if not keyboard_blocked:
                key_board_linux.block_keyboard(timeout)
...
def unblock_keyboard():
    ...
    try:
        with keyboard_lock:
            if keyboard_blocked:
                key_board_linux.unblock_keyboard()
                keyboard_blocked = False
    except Exception as e:
